...
import { getRaces, getNextRace, CURRENT_SEASON } from '@/lib/data/dataUtils';
import { getDriver, getTeam } from '@/lib/data/dataUtils';
import { DRIVERS, TEAMS } from '@/lib/data/mockData';

export const Predictions: React.FC = () => {
  const [season, setSeason] = useState(CURRENT_SEASON);
  const seasonProbs = getSeasonOutcomeProbabilities(season);
  const races = getRaces(season);
  const nextRace = getNextRace();
  const next3Races = races.filter((r) => !r.completed).slice(0, 3);
  ...
  const adjustedDriverProbs = useMemo(() => {
    return seasonProbs.driverTitle.map((prob) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-2xl p-6 border border-border">
          ...
          <div className="space-y-3 mb-4">
            {seasonProbs.driverTitle.slice(0, 5).map((prob) => {
              const driver = getDriver(prob.driverId);
              return (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {seasonProbs.driverTitle.map((prob) => {
                  const driver = getDriver(prob.driverId);
                  return (
        ...
        <div className="bg-card rounded-2xl p-6 border border-border">
          ...
          <div className="space-y-3 mb-4">
            {seasonProbs.constructorTitle.slice(0, 5).map((prob) => {
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {seasonProbs.constructorTitle.map((prob) => {
                  const team = getTeam(prob.teamId);
                  return (
      ...
      <div className="bg-card rounded-2xl p-6 border border-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {next3Races.map((race) => {
            const predictions = getRacePredictions(season, race.round);
            const predictedWinner = predictions.find((p) => p.expectedFinishPosition === 1);
          ...
          <div className="mt-6">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={adjustedDriverProbs.slice(0, 10).map((p) => ({
                  driver: getDriver(p.driverId)?.code || '',
                  original: seasonProbs.driverTitle.find((orig) => orig.driverId === p.driverId)?.titleProbability || 0,
